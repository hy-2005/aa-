...
仓库 README 使用名称 **OpenCluely**，`package.json` 的包名为 `windows-personal-assistant`，产品名称为“windows个人助手”。项目支持 Windows，并提供 macOS、Linux 的打包配置；实际 CI 构建矩阵只有 Windows 和 Linux。
...
5. **部分快捷键说明不一致**：截图队列实际发送键为 `Ctrl+Alt+D`，README 故障排查段落的 `Ctrl+Shift+D` 与注册代码不符。
